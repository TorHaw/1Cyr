...
* MIDN Toren Hawk
* Deadlock Lab
* Program edited by adding a single new binary semaphore only allowing one player to collect
* resources at a single time.
*/
...
semaphore collect_sem; // New semaphore
...
which resource to seek first. */
{
	...
	semWait(collect_sem); // wait to see if they can collect
    if (r == 0) {
	...
	semSignal(collect_sem); // release and update collection option
...
int main( int argc, char *argv[] )
{
	...
	semInit(collect_sem, NUM_GLOVES); // new semaphore initialize
	...
	semRelease(collect_sem); // release new semaphore
