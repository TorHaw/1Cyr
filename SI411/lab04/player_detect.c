		...
		while (some_just_ran) {
			...
			for (i=0; i<NUM_PLAYERS; i++) {
				...
				// If thread not done, see if resources are available for it to run
					...
					/* DONE:  For thread i, you must now loop over all resources.
					...
					/* write some code here!!! (based on the DONE above) */

					for (int j = 0; j < RESOURCE_COUNT; j++) {
						if (R_snapshot[i][j] > A[j]) can_run = false;
					}
					...
					if (can_run == false) {
					  ...
					  /* DONE: notionally 'run' thread i. Specifically, free its resources
...
int main( int argc, char *argv[] )
{
    ...
    /* DONE: Create deadlock detection thread */
    ...
    /* DONE: Wait for detector to finish. */
